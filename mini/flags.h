...
namespace mini
{
	...
	class flags final
	{
		...
		public:
			...
			template <typename U>
			typename std::enable_if<std::is_integral<U>::value, flags&>::type operator&=(U mask)
			{
				i &= mask;
				return *this;
			}
			...
			{
				return flags(static_cast<underlying_type>(~i));
			}
			...
			bool test_flag(T f) const
			{
				return (i & static_cast<underlying_type>(f)) != 0;
			}

			friend flags operator^(flags lhs, flags rhs)
			{
				return flags(static_cast<underlying_type>(lhs.i ^ rhs.i));
			}
			friend flags operator^(flags lhs, T f)
			{
				return flags(static_cast<underlying_type>(lhs.i ^ static_cast<underlying_type>(f)));
			}
			friend flags operator^(T     f, flags lhs)
			{
				return lhs ^ f;
			}
			...
			friend flags operator&(flags lhs, T rhs)
			{
				return flags(static_cast<underlying_type>(lhs.i & static_cast<underlying_type>(rhs)));
			}
			friend flags operator&(T     lhs, flags rhs)
			{
				return lhs & rhs;
			}
			...
			friend bool operator==(flags lhs, T rhs)
			{
				return lhs.i == static_cast<underlying_type>(rhs);
			}
			friend bool operator==(T lhs, flags rhs)
			{
				return rhs == lhs;
			}
			friend bool operator!=(flags lhs, T rhs)
